...
const b1 = new Audio('https://freesound.org/data/previews/99/99751_29308-lq.mp3');
const b2 = new Audio('https://freesound.org/data/previews/99/99752_29308-lq.mp3');
...
let alternateScroll = 0;  // Counter for alternate taps

const handleScroll = debounce(function() {
    if (alternateScroll % 2 === 0) {
        // Tap left paw for even count
        movePaws([false, true, true, false]);
    } else {
        // Tap right paw for odd count
        movePaws([true, false, false, true]);
    }

    alternateScroll++;  // Increase the counter
}, 30);  // Introduce a delay of 100ms between taps
...
function init() {
    ...
    const button = document.querySelector('.panda-button')
    ...
    const createAndAnimatePanda = () =>{
      ...
      panda.classList.add('panda-2')
    // Getting the difference in position between the button and the cover
    let topDifference = pandaFace.getBoundingClientRect().top - cover.getBoundingClientRect().top;
    let leftDifference = pandaFace.getBoundingClientRect().left - cover.getBoundingClientRect().left;

    panda.style.top = `${topDifference}px`;
    panda.style.left = `${leftDifference}px`;
    panda.style.zIndex = "500";
